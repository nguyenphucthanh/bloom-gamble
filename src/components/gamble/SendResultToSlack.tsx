...
import { PlayerAmount, PlayerKey, selectPayback, selectPlayer, selectPlayerPoint } from "./gambleSlice";
...
const SendResultToSlack: FC = () => {
    ...
    const paybackResult: string[] = []
     ...
     Array.from(paybacks.keys()).map((key) => {
      return paybacks
        ...
        paybackResult.push(`${player[payback.player as PlayerKey] ?? ""} chuyển cho ${player[key as PlayerKey] ?? ""} ${payback.amount}K`)
        );
    })
  ...
  return <button className="font-bold bg-blue-500 text-white rounded p-3 mt-5 text-center justify-center inline-flex gap-2 w-full col-span-2"
  onClick={sendResult}>Post Result To Slack</button>
}
